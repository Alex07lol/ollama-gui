...
interface ChatAreaProps {
  ...
  onUpdatePinnedCapabilities?: (pinned: string[]) => void;
  onUpdateDisabledCapabilities?: (disabled: string[]) => void;
}
...
export const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  onExecuteAction,
  stepLogs,
  onUpdatePinnedCapabilities,
  onUpdateDisabledCapabilities,
}) => {
  const [input, setInput] = useState('');
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'chat' | 'plan'>('chat');
  const [showCapsConfig, setShowCapsConfig] = useState(false);
  ...
  const availableCapabilities = [
    { id: 'workspace', name: 'Workspace Indexing' },
    { id: 'research', name: 'Documentation Research' },
    { id: 'open_source', name: 'Open Source Patterns' },
    { id: 'planning', name: 'Action Planning' },
    { id: 'build', name: 'Code Building' },
    { id: 'review', name: 'Senior Review' },
    { id: 'security', name: 'Security Audit' },
  ];

  const pinned = conversation.pinnedCapabilities || [];
  const disabled = conversation.disabledCapabilities || [];

  const handleTogglePinned = (capId: string) => {
    if (!onUpdatePinnedCapabilities) return;
    if (pinned.includes(capId)) {
      onUpdatePinnedCapabilities(pinned.filter(c => c !== capId));
    } else {
      onUpdatePinnedCapabilities([...pinned, capId]);
      // Remove from disabled if it was there
      if (onUpdateDisabledCapabilities && disabled.includes(capId)) {
        onUpdateDisabledCapabilities(disabled.filter(c => c !== capId));
      }
    }
  };

  const handleToggleDisabled = (capId: string) => {
    if (!onUpdateDisabledCapabilities) return;
    if (disabled.includes(capId)) {
      onUpdateDisabledCapabilities(disabled.filter(c => c !== capId));
    } else {
      onUpdateDisabledCapabilities([...disabled, capId]);
      // Remove from pinned if it was there
      if (onUpdatePinnedCapabilities && pinned.includes(capId)) {
        onUpdatePinnedCapabilities(pinned.filter(c => c !== capId));
      }
    }
  };
  ...
  return (
    <div className="main-content">
      ...
      {/* Visual Dynamic Capability Pipeline (If Intelligent selection is active) */}
      {conversation.activePipeline && (
        <div 
          style={{ 
            padding: '10px 14px', 
            backgroundColor: 'var(--bg-secondary)', 
        ...
        >
          <div className="flex justify-between items-center" style={{ fontSize: '10.5px' }}>
            <div className="flex items-center gap-1.5">
              <span style={{ fontWeight: 600, color: 'var(--status-blue)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                ✓ Intelligent Pipeline
              </span>
              <span style={{ color: 'var(--text-muted)' }}>— {conversation.activePipeline.analysisReason}</span>
            </div>
            <span style={{ color: 'var(--text-muted)' }}>
              Duration: <strong style={{ color: 'var(--text-primary)' }}>{conversation.activePipeline.estimatedDuration}</strong>
            </span>
          </div>

          <div className="flex gap-2 items-center" style={{ overflowX: 'auto', padding: '2px 0' }}>
            {conversation.activePipeline.capabilities.map((cap, i) => (
              <div key={cap.id} className="flex items-center gap-1.5">
                <div 
                  style={{ 
                    padding: '3px 8px', 
                    backgroundColor: cap.status === 'running' ? 'rgba(59, 130, 246, 0.08)' : 'var(--bg-primary)',
                    border: `1px solid ${cap.status === 'running' ? 'var(--status-blue)' : 'var(--border-primary)'}`,
                    borderRadius: '4px',
                    fontSize: '11px',
                    color: cap.status === 'completed' ? 'var(--text-primary)' : cap.status === 'running' ? 'var(--status-blue)' : 'var(--text-muted)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '4px'
                  }}
                ...
                >
                  {cap.status === 'completed' && <span style={{ color: 'var(--status-green)', fontWeight: 'bold' }}>✓</span>}
                  {cap.status === 'running' && <span className="streaming-dot" />}
                  {cap.status === 'skipped' && <span style={{ color: 'var(--text-disabled)' }}>⊘</span>}
                  {cap.status === 'pending' && <span style={{ color: 'var(--text-disabled)', fontSize: '8px' }}>○</span>}
                  <span>{cap.name}</span>
                </div>
                {i < (conversation.activePipeline.capabilities.length - 1) && (
      ...
      {/* Capability Drawer Overlay (Manual overrides) */}
      {showCapsConfig && (
        <div 
          style={{ 
            padding: '14px 20px', 
        ...
        >
          <div className="flex justify-between items-center">
            ...
            <button 
              onClick={() => setShowCapsConfig(false)}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '8px' }}>
            {availableCapabilities.map((cap) => {
              const isPinned = pinned.includes(cap.id);
              const isDisabled = disabled.includes(cap.id);
              ...
              return (
                ...
                >
                  <span style={{ fontSize: '11.5px', fontWeight: 600, color: 'var(--text-primary)' }}>{cap.name}</span>
                  <div className="flex gap-2" style={{ marginTop: '2px' }}>
                    <button 
                      onClick={() => handleTogglePinned(cap.id)}
                      style={{ 
                        padding: '2px 6px', 
                        fontSize: '9.5px', 
                        border: '1px solid var(--border-primary)',
                        borderRadius: '3px',
                        backgroundColor: isPinned ? 'rgba(34, 197, 94, 0.1)' : 'transparent',
                        color: isPinned ? 'var(--status-green)' : 'var(--text-muted)',
                        cursor: 'pointer'
                      }}
                    >
                      {isPinned ? '★ Pinned' : 'Pin'}
                    ...
                    <button 
                      onClick={() => handleToggleDisabled(cap.id)}
                      style={{ 
                        padding: '2px 6px', 
                        fontSize: '9.5px', 
                        border: '1px solid var(--border-primary)',
                        borderRadius: '3px',
                        backgroundColor: isDisabled ? 'rgba(239, 68, 68, 0.1)' : 'transparent',
                        color: isDisabled ? 'var(--status-red)' : 'var(--text-muted)',
                        cursor: 'pointer'
                      }}
                    >
      ...
      >
        ...
        <div className="flex justify-between items-center" style={{ fontSize: '11px', color: 'var(--text-muted)', padding: '0 2px' }}>
          <div className="flex gap-3 items-center">
            ...
            {/* Pinned Capabilities Override selector trigger */}
            <button 
              onClick={() => setShowCapsConfig(!showCapsConfig)}
              style={{ border: 'none', background: 'transparent', padding: '2px 6px', fontSize: '10px', color: 'var(--text-muted)', textDecoration: 'underline', cursor: 'pointer' }}
            >
              Capabilities {pinned.length > 0 ? `(${pinned.length} pinned)` : ''}
            </button>
